...
def load_word_data(label='training_label.txt', unlabel='training_nolabel.txt'):
    label_data = open(label).read()
    nobel_data = open(unlabel).read()
...
def load_train_data(label='training_label.txt', no_comma=False):
    label_data = open(label).read()
    if no_comma:
        gex = re.compile("[\!,\.:;\?\'\"]")
...
def train_word2vec(label,unlabel):
    sentences = load_word_data(label,unlabel)
...
def train_rnn(label='training_label.txt',no_comma=False):
    ...
    x, y = load_train_data(label=label,no_comma=no_comma)
...
if __name__ == "__main__":
    ...
    # train_word2vec(label=sys.argv[1],unlabel=sys.argv[2])
    train_rnn(label=sys.argv[1])
    # test()